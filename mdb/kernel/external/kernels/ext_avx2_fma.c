...
static inline bool query_str_app(const char* src, char* dst, size_t size, size_t* offset)
{
...
void mdb_kernel_ext_metadata_query(int flags, char* buff, uint32_t buff_size)
{
    size_t offset = 0;

    if(flags & MDB_KERNEL_EXT_META_NAME)
        if(!query_str_app(name, buff, buff_size, &offset))
            return;

    if(flags & MDB_KERNEL_EXT_META_VER_MAJ)
        if(!query_str_app(ver_maj, buff, buff_size, &offset))
            return;

    if(flags & MDB_KERNEL_EXT_META_VER_MIN)
        if(!query_str_app(ver_min, buff, buff_size, &offset))
            return;

}

void mdb_kernel_ext_init(void)
{

}

void mdb_kernel_ext_shutdown(void)
{

}

void mdb_kernel_ext_set_size(uint32_t width, uint32_t height)
{
    mdb.width = width;
    mdb.height = height;
...
void mdb_kernel_ext_set_scale(float scale)
{
    mdb.scale = scale;
}

void mdb_kernel_ext_set_shift(float shift_x, float shift_y)
{
    mdb.shift_x = shift_x;
    mdb.shift_y = shift_y;
}
...
void mdb_kernel_ext_set_bailout(uint32_t bailout)
{
    mdb.bailout = bailout;
}

void mdb_kernel_ext_set_surface(float* buffer)
{
    mdb.f32surface = buffer;
}

void mdb_kernel_ext_submit_changes(void)
{
}

void mdb_kernel_ext_process_block(uint32_t x0, uint32_t x1, uint32_t y0, uint32_t y1)
{
